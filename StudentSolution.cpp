...
// int findCanalToOpen(std::vector<Canal *> canals, std::string region)
// {
// 	int match;
// 	for(int i = 0; i< canals.size();i++)
// 	{
...
// 			{
// 				int index =  findCanalToOpen(canals, regions[i]->name);
// 				canals[i]->toggleOpen(true);
// 				canals[i]->setFlowRate(1);
...
void solveProblems(AcequiaManager& manager)
{
    ...
    while(!manager.isSolved && manager.hour != manager.SimulationMax)
    {
        ...
        for(int i = 0; i < (int)canals.size(); i++)
        {
            ...
            double droughtLimit = 0.2 * src->waterCapacity;
            double floodLimit = dst->waterCapacity;

			if(src->waterLevel > droughtLimit && dst->waterLevel < floodLimit){
				double flow = 1.0;
